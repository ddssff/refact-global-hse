...
instance HasSymbols S.Decl where
    ...
    toExportSpecs x@(TypeFamDecl _SrcLoc _Name _TyVarBinds _MaybeKind) = error $ "HasSymbols TypeFamDecl: " ++ prettyPrint x
    toExportSpecs x@(ClosedTypeFamDecl _SrcLoc _Name __TyVarBind _MaybeKind _TypeEqns) = error $ "HasSymbols ClosedTypeFamDecl: " ++ prettyPrint x
    ...
    toExportSpecs x@(GDataDecl _SrcLoc _DataOrNew _Context _Name _TyVarBind _MaybeKind _GadtDecls _Deriving) = error $ "HasSymbols GDataDecl: " ++ prettyPrint x
    toExportSpecs x@(DataFamDecl _SrcLoc _Context _Name _TyVarBind _MaybeKind) = error $ "HasSymbols DataFamDecl: " ++ prettyPrint x
    toExportSpecs x@(TypeInsDecl _SrcLoc _Type1 _Type2) = error $ "HasSymbols TypeInsDecl: " ++ prettyPrint x
    toExportSpecs x@(DataInsDecl _SrcLoc _DataOrNew _Type _QualConDecls _Derivings) = error $ "HasSymbols DataInsDecl: " ++ prettyPrint x
    toExportSpecs x@(GDataInsDecl _SrcLoc _DataOrNew _Type _MaybeKind _GadtDecls _Deriving) = error $ "HasSymbols GDataInsDecl: " ++ prettyPrint x
    toExportSpecs (ClassDecl _SrcLoc _Context name _TyVarBind _FunDeps classDecls) =
        case concatMap toCNames classDecls of
          [] -> [S.EVar (S.UnQual name)]
          xs -> [S.EThingWith (S.UnQual name) xs]
    toExportSpecs x@(InstDecl _SrcLoc _MaybeOverlap _TyVarBind _Context _QName _Types _InstDecls) = error $ "HasSymbols InstDecl: " ++ prettyPrint x
    toExportSpecs x@(DerivDecl _SrcLoc _MaybeOverlap _TyVarBind _Context _QName _Types) = error $ "HasSymbols DerivDecl: " ++ prettyPrint x
    toExportSpecs x@(InfixDecl _SrcLoc _Assoc _Int _Ops) = error $ "HasSymbols InfixDecl: " ++ prettyPrint x
    toExportSpecs x@(DefaultDecl _SrcLoc _Types) = error $ "HasSymbols DefaultDecl: " ++ prettyPrint x
    toExportSpecs x@(SpliceDecl _SrcLoc _Exp) = error $ "HasSymbols SpliceDecl: " ++ prettyPrint x
    toExportSpecs (TypeSig _SrcLoc names _Type) = map (S.EVar . S.UnQual) names
    toExportSpecs x@(PatSynSig _SrcLoc _Name _MaybeTyVarBind _Context1 _Context2 _Type) = error $ "HasSymbols PatSynSig: " ++ prettyPrint x
    toExportSpecs (FunBind matches) = concatMap toExportSpecs matches
    toExportSpecs x@(PatBind _SrcLoc _Pat _Rhs _MaybeBinds) = error $ "HasSymbols PatBind: " ++ prettyPrint x
    toExportSpecs x@(ForImp _SrcLoc _CallConv _Safety _String _Name _Type) = error $ "HasSymbols ForImp: " ++ prettyPrint x
    toExportSpecs x@(ForExp _SrcLoc _CallConv _String _Name _Type) = error $ "HasSymbols ForExp: " ++ prettyPrint x
    toExportSpecs x@(PatSyn _SrcLoc _Pat1 _Pat2 _PatternSynDirection) = error $ "HasSymbols PatSyn: " ++ prettyPrint x
    toExportSpecs x@(RulePragmaDecl _SrcLoc _Rules) = error $ "HasSymbols RulePragmaDecl: " ++ prettyPrint x
    toExportSpecs x@(DeprPragmaDecl _SrcLoc _pairs {-[([Name], String)]-}) = error $ "HasSymbols DeprPragmaDecl: " ++ prettyPrint x
    toExportSpecs x@(WarnPragmaDecl _SrcLoc _pairs {-[([Name], String)]-}) = error $ "HasSymbols WarnPragmaDecl: " ++ prettyPrint x
    toExportSpecs x@(InlineSig _SrcLoc _Bool _Activation _QName) = error $ "HasSymbols InlineSig: " ++ prettyPrint x
    toExportSpecs x@(InlineConlikeSig _SrcLoc _Activation _QName) = error $ "HasSymbols InlineConlikeSig: " ++ prettyPrint x
    toExportSpecs x@(SpecSig _SrcLoc _Activation _QName _Types) = error $ "HasSymbols SpecSig: " ++ prettyPrint x
    toExportSpecs x@(SpecInlineSig _SrcLoc _Bool _Activation _QName _Types) = error $ "HasSymbols SpecInlineSig: " ++ prettyPrint x
    toExportSpecs x@(InstSig _SrcLoc _TyVarBind _Context _QName _Types) = error $ "HasSymbols InstSig: " ++ prettyPrint x
    toExportSpecs x@(AnnPragma _SrcLoc _Annotation) = error $ "HasSymbols AnnPragma: " ++ prettyPrint x
    toExportSpecs x@(MinimalPragma _SrcLoc _MaybeBooleanFormula) = error $ "HasSymbols MinimalPragma: " ++ prettyPrint x
    toExportSpecs x@(RoleAnnotDecl _SrcLoc _QName _Roles) = error $ "HasSymbols RoleAnnotDecl: " ++ prettyPrint x
...
instance HasCNames S.ClassDecl where
    toCNames (S.ClsDecl decl) =
        ...
        where
          ...
          f _ = []
    toCNames x@(S.ClsDataFam _SrcLoc _Context _name _TyVarBinds _MaybeKind) = error $ "ClsDataFam: " ++ prettyPrint x
    toCNames x@(S.ClsTyFam _SrcLoc _name _TyVarBinds _MaybeKind) = error $ "ClsTyFam: " ++ prettyPrint x
    toCNames x@(S.ClsTyDef _SrcLoc _Type1 _Type2) = error $ "ClsTyDef: " ++ prettyPrint x
    toCNames x@(S.ClsDefSig _SrcLoc _name _type) = error $ "ClsDefSig: " ++ prettyPrint x
