...
import qualified Language.Haskell.Exts.Syntax as S (CName(VarName), ExportSpec(EThingWith, EVar), ImportDecl(importLoc, importModule, importSpecs), ImportSpec(IThingWith, IVar), ModuleName(..), Name(..), QName(UnQual))
import SrcLoc (srcLoc)
import Symbols (FoldDeclared(..), FoldMembers, members)
import System.Exit (ExitCode(ExitSuccess, ExitFailure))
import System.FilePath ((</>))
import System.FilePath.Extra2 (replaceFile)
import System.Process (readProcessWithExitCode, showCommandForUser)
import Types (DerivDeclTypes(derivDeclTypes), hseExtensions, hsFlags, loadModule, ModuleInfo(ModuleInfo, _module, _moduleKey, _modulePath, _moduleText), ModuleKey(_moduleTop))
