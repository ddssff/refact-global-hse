...
import Prelude (($), (++), (.), (<$>), Applicative(pure), Bool(False), Enum(fromEnum, pred, succ, toEnum), Eq((==)), error, fst, id, Int, Monad((>>=), return), Num((*), (+)), Ord((<)), Show(show), snd, zip)
import Prelude hiding (map)
import Test.HUnit (Test(..), Test(TestList, TestLabel, TestCase))
