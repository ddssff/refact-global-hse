...
import Data.Foldable as Foldable (find, Foldable(foldr, foldr1), maximumBy)
import Data.Function (on)
import qualified Data.List as List (map)
import Data.Map.Strict as Map (delete, findMin, insert, lookup, Map, member, singleton)
import Data.Maybe (catMaybes, fromMaybe, isNothing)
import Data.Monoid ((<>))
import Data.Sequence as Seq ((><), Seq, singleton, viewl, ViewL(EmptyL, (:<)))
...
import Prelude (($), (++), (.), (<$>), all, any, Bool(..), const, Either(..), Enum(fromEnum, succ, toEnum), Eq((==)), flip, Foldable, Fractional((/)), fst, Functor(fmap), Int, IO, mapM_, maybe, Maybe(..), Monad(..), Monoid(mempty), Num((-)), Ord((>), compare), Ordering, Rational, Read, realToFrac, Show(show), String, Traversable(mapM), unlines)
