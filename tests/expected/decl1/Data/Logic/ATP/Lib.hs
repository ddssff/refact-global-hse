...
import Prelude (($), (++), (.), (<$>), all, any, Bool(..), const, Either(..), Enum(toEnum, succ, fromEnum), Eq((==)), flip, Fractional((/)), fst, Functor(fmap), Int, IO, mapM_, maybe, Maybe(..), Monad(..), Monoid(mempty), Num((-)), Ord((>), compare), Ordering, Rational, Read, realToFrac, Show(show), String, Traversable(mapM), unlines)
